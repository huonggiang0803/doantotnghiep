...
public class PdfService {
    ...
    public byte[] generateInvoice(Bill bill) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            document.add(new Paragraph("Khách hàng: " + bill.getUserEntity().getUsername()).setFont(font));
            ...
            table.addCell(new Cell().add(new Paragraph("Thành tiền").setBold().setFont(font)));
    ...
    public PdfFont loadFont() {
        try {
            InputStream fontStream = getClass().getClassLoader().getResourceAsStream("fonts/Times New Normal Regular.ttf");
            if (fontStream == null) {
